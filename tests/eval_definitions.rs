...
fn define_with_one_arg() {
    assert_eval_vm_val("(define x)", unspecified());
    assert_eval_vm_val("(define x)
                        x", unspecified());
}

#[test]
fn define_with_two_args() {
    assert_eval_vm_val("(define x 2)", unspecified());

    assert_eval_vm("(define x 2)
                    (= x 2)", "#t");
    assert_eval_vm("(define x 9)
                    (define x 10)
                    x", "10");
    assert_eval_vm("(define x 9)
                    (define y x)
...
#[test]
fn define_bad_arity() {
    assert_eval_vm_err("(define)", malformed_expr());
}
