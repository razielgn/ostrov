...
fn execute_load_constant() {
    {
        ...
        assert_eq!(Ok(vm.memory.integer(3)), vm.execute(instr.into_iter()));
    }

    {
        ...
        assert_eq!(Ok(vm.memory.b_false()), vm.execute(instr.into_iter()));
    }
}
...
fn execute_jump() {
    {
        ...
        assert_eq!(Ok(vm.memory.integer(23)), vm.execute(instr.into_iter()));
    }
}
...
fn execute_jump_on_false() {
...

    {
        ...
        assert_eq!(Ok(vm.memory.integer(1)), vm.execute(instr.into_iter()));
    }
}
...
fn execute_jump_on_true() {
...

    {
        ...
        assert_eq!(Ok(vm.memory.b_true()), vm.execute(instr.into_iter()));
    }
}
...
fn execute_load_reference() {
    {
        let mut vm = VM::new();
        let instr = vec![Instruction::load_reference("a".to_owned())];
...
fn execute_load_unspecified() {
    ...
    assert_eq!(Ok(vm.memory.unspecified()), vm.execute(instr.into_iter()));
}
...
fn execute_apply() {
    ...
    assert_eq!(Ok(vm.memory.integer(0)), vm.execute(instr.into_iter()));
}
...
fn execute_argument() {
    ...
    assert_eq!(Ok(vm.memory.integer(2)), vm.execute(instr.into_iter()));
}
...
fn execute_nested_arguments() {
    ...
    assert_eq!(Ok(vm.memory.integer(4)), vm.execute(instr.into_iter()));
}
...
fn illegal_frame_apply() {
    ...
    assert_eq!(Err(cannot_pop_last_frame()), vm.execute(instr.into_iter()));
}
