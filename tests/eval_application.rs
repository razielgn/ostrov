...
fn procedure_with_one_arg() {
    assert_eval_vm("(define (x y) y)
                    (x (+ 2 9))", "11");
}

#[test]
fn procedure_with_two_args() {
    assert_eval_vm("(define (sum x y) (+ x y))
                    (sum 4 9)", "13");
}

#[test]
fn procedure_with_var_args() {
    assert_eval_vm("(define (sum x . y) y)
                    (sum 4 9)", "'(9)");
}

#[test]
fn procedure_with_any_args() {
    assert_eval_vm("(define (sum . y) y)
                    (sum 4 9)", "'(4 9)");
}

#[test]
fn procedure_with_two_args_scoping() {
    assert_eval_vm("(define x 10)
                    (define (sum x y) (+ x y))
                    (sum 4 9)", "13");
}
...
fn procedure_with_mismatched_arity() {
    assert_eval_vm_err("(define (sum x y) (+ x y))
                        (sum 4)", bad_arity("sum"));
}

#[test]
fn lambda_multiple_expressions_are_evaluated_sequentially() {
    assert_eval_vm("(define (foo)
                      (define a 10)
                      (define b (+ a 1))
                      (define c (+ b 1))
                      c)
...
fn lambda_with_fixed_arguments_number() {
    ...
    assert_eval_vm("((lambda () 1 2 3))", "3");
    assert_eval_vm("((lambda (x y) (+ x y)) 6 8)", "14");
}

#[test]
fn lambda_with_fixed_arguments_number_bad_arity() {
    assert_eval_vm_err("((lambda (x y) (+ x y)) 6 8 9)", bad_arity_lambda());
}

#[test]
fn lambda_with_any_arguments_number() {
    assert_eval_vm("((lambda x x))", "'()");
    assert_eval_vm("((lambda x x) 1 2 3)", "'(1 2 3)");
}

#[test]
fn lambda_with_variable_arguments_number() {
    assert_eval_vm("((lambda (x . y) x) 1)", "1");
    assert_eval_vm("((lambda (x . y) y) 1)", "'()");
    assert_eval_vm("((lambda (x . y) y) 1 2 3)", "'(2 3)");
    assert_eval_vm("((lambda (x y . z) z) 1 2 3)", "'(3)");
}

#[test]
fn lambda_with_variable_arguments_number_bad_arity() {
    assert_eval_vm_err("((lambda (x . y) 1))", bad_arity_lambda());
    assert_eval_vm_err("((lambda (x y . z) 1) 1)", bad_arity_lambda());
}
