...
pub mod ast {
    ...
    pub fn atom(val: &str)     -> AST { AST::Atom(val.to_owned()) }
...
pub mod values {
    ...
    pub fn atom(val: &str) -> RcValue { Rc::new(Value::Atom(val.to_owned())) }
...
pub fn unbound_variable_error(val: &str) -> Error { Error::UnboundVariable(val.to_owned()) }
...
pub fn bad_arity(val: &str) -> Error { Error::BadArity(Some(val.to_owned())) }
